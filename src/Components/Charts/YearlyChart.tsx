...
const YearlyChart: React.FC<Data> = ({ deviceName, startDate, endDate }) => {
  const [data, setData] = useState<Data | any>();
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(
          `http://localhost:3001/consumption/yearly?deviceName=${deviceName}&startDate=${startDate}&endDate=${endDate}`
          // {
          //   method: "GET",
          //   headers: {
